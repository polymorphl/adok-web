...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		this.ctrlContinue = createElement( 'button', { cName : 'fs-continue', inner : 'Continuer', appendTo : this.ctrls } );
	...
	FForm.prototype._deleteEvents = function() {
		var self = this;

		// show next field
		this.ctrlContinue.removeEventListener( 'click', function() {
			self._nextField();
		} );
		...
		this.fields.forEach( function( fld ) {
			if( fld.hasAttribute( 'data-input-trigger' ) ) {
				...
				switch( input.tagName.toLowerCase() ) {
					case 'select' :
						input.removeEventListener( 'change', function() { self._nextField(); } );
						break;

					case 'input' :
						[].slice.call( fld.querySelectorAll( 'input[type="radio"]' ) ).forEach( function( inp ) {
							inp.removeEventListener( 'change', function(ev) { self._nextField(); } );
						} );
						break;
	...
	FForm.prototype.destroy = function() {
		this._deleteEvents();
		this.ctrls.remove();
		this.msgError.remove();
		classie.remove( this.formEl, 'fs-form-overview' );
		classie.add( this.formEl, 'fs-form-full' );
		classie.remove( this.formEl, 'fs-show' );
		return this;
	}
