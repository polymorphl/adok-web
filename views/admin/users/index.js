...
exports.create = function(req, res, next){
  ...
  workflow.on('validate', function() {
    if (!req.body.username) {
      workflow.outcome.errors.push('Entrer un nom d\'utilisateur.');
      return workflow.emit('response');
    }

    if (!/^[a-zA-Z0-9\-\_]+$/.test(req.body.username)) {
      workflow.outcome.errors.push('Utiliser des lettres, nombres, -, _');
      return workflow.emit('response');
    }
  ...
  workflow.on('duplicateUsernameCheck', function() {
    req.app.db.models.User.findOne({ username: req.body.username }, function(err, user) {
      ...
      if (user) {
        workflow.outcome.errors.push('Ce nom d\'utilisateur est déjà pris.');
        return workflow.emit('response');
      }
...
exports.update = function(req, res, next){
  ...
  workflow.on('validate', function() {
    ...
    if (!req.body.username) {
      ...
      workflow.outcome.errfor.username = 'Utiliser des lettres, nombres, \'-\', \'_\'';
    }
    ...
    if (!req.body.email) {
      ...
      workflow.outcome.errfor.email = 'Format d\'adresse e-mail invalide';
    }
...
exports.linkAdmin = function(req, res, next){
  ...
  workflow.on('validate', function() {
    ...
    if (!req.body.newAdminId) {
      workflow.outcome.errfor.newAdminId = req.i18n.t('errors.required');
      return workflow.emit('response');
    }
...
exports.linkAccount = function(req, res, next){
  ...
  workflow.on('validate', function() {
    ...
    if (!req.body.newAccountId) {
      workflow.outcome.errfor.newAccountId = req.i18n.t('errors.required');
      return workflow.emit('response');
    }
