...
var renderSettings = function(req, res, next, oauthMessage) {
  ...
  var getUserData = function(callback) {
    req.app.db.models.User.findById(req.user.id, 'username email twitter.id google.id facebook.id').exec(function(err, user) {
      if (err) {
        callback(err, null);
      }
...
exports.connectTwitter = function(req, res, next){
  req._passport.instance.authenticate('twitter', function(err, user, info) {
    ...
    req.app.db.models.User.findOne({ 'twitter.id': info.profile._json.id, _id: { $ne: req.user.id } }, function(err, user) {
      ...
      if (user) {
        renderSettings(req, res, next, req.i18n.t('social.alreadyused')+' Twitter.');
      }
      else {
...
exports.connectGoogle = function(req, res, next){
  req._passport.instance.authenticate('google', { callbackURL: '/account/settings/google/callback/', scope: ['https://www.googleapis.com/auth/userinfo.profile','https://www.googleapis.com/auth/userinfo.email'] }, function(err, user, info) {
    if (!info || !info.profile) {
      return res.redirect('/account/settings/');
    }

    req.app.db.models.User.findOne({ 'google': info.profile._json.id, _id: { $ne: req.user.id } }, function(err, user) {
      if (err) {
        return next(err);
      }

      if (user) {
        renderSettings(req, res, next, req.i18n.t('social.alreadyused')+' Google.');
      }
      else {
...
exports.connectFacebook = function(req, res, next){
  req._passport.instance.authenticate('facebook', { callbackURL: '/account/settings/facebook/callback/' }, function(err, user, info) {
    ...
    req.app.db.models.User.findOne({ 'facebook.id': info.profile._json.id, _id: { $ne: req.user.id } }, function(err, user) {
      ...
      if (user) {
        renderSettings(req, res, next, req.i18n.t('social.alreadyused')+' Facebook.');
      }
      else {
...
exports.password = function(req, res, next){
  ...
  workflow.on('validate', function() {
    if (!req.body.newPassword) {
      workflow.outcome.errfor.newPassword = req.i18n.t('errors.required');
    }
